...
/// Outcome of resolving a `ForkSignal` (WP-6).
...
impl Model {
    ...
    /// The hash at position `n` — a stored block, or the anchor (whose hash may be `⊥`).
    /// `None` for a slot that produced no block.
    pub fn hash_at(&self, n: BlockNumber) -> Option<&str> {
        if let Ok(i) = self.seg.binary_search_by_key(&n, |b| b.number) {
            return Some(&self.seg[i].hash);
        }
    ...
    fn finalize_inner(&mut self, r: &BlockRef) -> Finalize {
        ...
        let at_e = self.hash_at(e).expect("e is within the window");
        // The report names a stored block, but a different one — an integrity fault (WP-8).
        if e == r.number && at_e != r.hash {
        ...
        let new = BlockRef::new(e, at_e.to_string());
    ...
    pub fn predict_query(
        ...
        let hi = if finalized_only {
            self.fin.as_ref().map(|f| f.number)
        } else {
...
mod tests {
    ...
    fn sparse_numbering_is_still_one_chain() {
        ...
        assert_eq!(m.hash_at(101), None, "a skipped slot holds no block");
    ...
    fn retain_trims_moves_the_anchor_and_drops_stale_finality() {
        ...
        // Trimming at or below the floor is a no-op.
        m.retain(105, None);
        assert_eq!(m.first(), Some(105));
        m.retain(100, None);
        assert_eq!(m.first(), Some(105));
