...
hash_at(p) = hash of the highest stored block with number ≤ p,
             or anchor.hash when no stored block lies at or below p
```
...
`expected_parent` (DEF-13), CONFLICT hints ([RP-11](04-read-path.md)), continuation
cursors ([RP-10](04-read-path.md)) — a pair `⟨p, h⟩` reads "`h` is the hash of the
preceding block of `p + 1`" (`h = hash_at(p)`), **not** "a block numbered `p` has hash
`h`". The two readings coincide exactly on densely-numbered chains.
...
| Policy | Meaning |
|---|---|
| `Window(k)` | Keep (at least) all stored blocks in the last `k` **positions** below `next(D)`; trim the rest as the head advances. `k` counts numbers, not stored blocks: on a slot-numbered chain the window holds ≤ `k` blocks. |
...
| `ForkSignal(hints)` | non-empty ascending list of Refs | the source disagrees with the service's requested position; `hints` are recent blocks of the source's canonical chain near the divergence point |
...
Each transition is atomic and increments `ver` exactly once. A single commit MAY compose
`EXTEND`/`REPLACE` with `FINALIZE` (blocks and a finality advance arriving together);
invariants are evaluated at commit points only.
