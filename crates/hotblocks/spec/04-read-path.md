...
- Live query: `hi(S) = head(S).number`; finalized-only: `hi(S) = fin(S).number`
  (undefined if `fin = ⊥`). Effective range: `[from, min(to, hi(S))]` with `to = ∞` when
  omitted.
...
- **RP-6 (Finalized-only without finality).** `QUERY-FINALIZED` when `fin = ⊥` behaves as
  `from > hi`: wait, then `NO_DATA`. It MUST NOT be an internal error.
