...
- **IB-4** Success body: one compressed stream whose decompressed content is
  **JSON Lines** — exactly one JSON object per emitted block, in ascending order:
  ...
  §4-of-12 validators parse it generically.
...
| `RANGE_UNAVAILABLE` | 400 today; distinct machine-readable discrimination from other 400s is REQUIRED for conformance clients (SHOULD: structured error body) |
| `ITEM_UNAVAILABLE` | 400 (same note) |
| `KIND_MISMATCH` | 400 |
...
Unbounded. (Config-level `Api` marks a dataset as External-mode.)
