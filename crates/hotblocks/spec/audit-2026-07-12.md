...
> **RESOLVED (2026-08-10, upstream convergence).** Upstream fixed this class in
> `2a80c8e fix(hotblocks): enforce finalized fork floor` (building on the `3bbb9a7`
> head/finalized-head consolidation), merged into this fork at `0182f4d`. The shipped
> mechanism differs from the one prescribed above: `compute_rollback` now rejects hints
> below or conflicting with the finalized head and carries a `reach_at_least` floor,
> resume happens at a physical chunk boundary at/below `fin`, and a replacement at/below
> finality is admitted only when it spans `fin` and reproduces the stored finalized
> prefix block-for-block — otherwise committed chunks, indexes, head and finalized head
> stay untouched and a genuine deeper divergence surfaces as an explicit RESET/refusal.
> This is the mechanism the current normative spec sanctions (WP-6 fork resolution,
> INV-14 fork floor in `06-invariants.md`). The vulnerability description above remains
> accurate for the pre-fix code. Regression coverage: `tests/ct4_finality.rs`,
> `tests/ct9_source_faults.rs`.
